								...
								<div class="card card-flush py-4">
									...
									<div class="card-body pt-0">
										...
										<div class="mb-10 fv-row">
											...
											<textarea id="content_description" class="form-control">{{ $data->content }}</textarea>
										</div>
										<div class="mb-10 fv-row">
...
<script>
	...
	function submitContent(){
  	...
  	function saveData(){
		...
		data.append('item_id', '{{$data->id}}');
		data.append('titleprofile', $('.titleprofile').val());
		data.append('content_description', $('#content_description').val());
		data.append('priority',$('.priority').val());
		data.append('status', $('#status').val());
		var response = adminAjax('{{route("ajax.save.years")}}', data);
		if(response.status == '200'){
			swal.fire({ type: 'success', title: response.message, showConfirmButton: false, timer: 1500 });
			setTimeout(function(){ location.reload(); }, 2000)
