	...
	<div class="toolbar" id="kt_toolbar">
		<div id="kt_toolbar_container" class="container-fluid d-flex flex-stack">
			<div data-kt-swapper="true" data-kt-swapper-mode="prepend" data-kt-swapper-parent="{default: '#kt_content_container', 'lg': '#kt_toolbar_container'}" class="page-title d-flex align-items-center flex-wrap me-3 mb-5 mb-lg-0">
				<h1 class="d-flex text-dark fw-bolder fs-3 align-items-center my-1">Popup Customer Details</h1>
			</div>
...
<script>
	...
	$(function () {
		...
		var table = $('.data-tbl').DataTable({
			...
			serverSide: true,
			ajax: {
				url: "{{route('ajax.user.service.list')}}",
				method: "POST",
				"data" : {
					"_token": "{{ csrf_token() }}",
				}
	...
	function deleteThis(item_id = ''){
		if(confirm("{{trans('common.delete_confirm')}}")){
			...
			var response = adminAjax('{{route("ajax.user.service.delete")}}', data);
