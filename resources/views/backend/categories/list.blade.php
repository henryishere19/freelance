	...
	<div class="toolbar" id="kt_toolbar">
		<div id="kt_toolbar_container" class="container-fluid d-flex flex-stack">
			<div data-kt-swapper="true" data-kt-swapper-mode="prepend" data-kt-swapper-parent="{default: '#kt_content_container', 'lg': '#kt_toolbar_container'}" class="page-title d-flex align-items-center flex-wrap me-3 mb-5 mb-lg-0">
				<h1 class="d-flex text-dark fw-bolder fs-3 align-items-center my-1">{{ trans('category.heading') }}</h1>
			</div>
			<div class="d-flex align-items-center gap-2 gap-lg-3">
				...
				<a href="{{ route('category.add', [$module]) }}" class="btn btn-sm btn-primary">Create</a>
			</div>
		</div>
	</div>
@endsection
...
<script type="text/javascript">
	$(document).ready(function() {
		...
		$(document).on('change','.change_status',function(){
			...
			var response = adminAjax('{{route("ajax.category.change.status")}}', data);
	...
	function deleteThis(item_id = ''){
		if(confirm("{{trans('common.delete_confirm')}}")){
			...
			var response = adminAjax('{{route("ajax.category.delete")}}', data);
			if(response.status == '200'){
				swal.fire({ type: 'success', title: response.message, showConfirmButton: false, timer: 1500 });
