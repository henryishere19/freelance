	...
	<div class="toolbar" id="kt_toolbar">
		<div id="kt_toolbar_container" class="container-fluid d-flex flex-stack">
			<div data-kt-swapper="true" data-kt-swapper-mode="prepend" data-kt-swapper-parent="{default: '#kt_content_container', 'lg': '#kt_toolbar_container'}" class="page-title d-flex align-items-center flex-wrap me-3 mb-5 mb-lg-0">
				<h1 class="d-flex text-dark fw-bolder fs-3 align-items-center my-1">Area Management</h1>
			</div>
				...
				<div class="modal-body scroll-y px-10 px-lg-15 pt-0 pb-15">
					<form id="formData" class="form" action="javascript:void(0);" onsubmit="saveData();">
						<div class="mb-13 text-center">
							<h1 class="mb-3">Create Area</h1>
						</div>
						<div class="d-flex flex-column mb-8 fv-row">
							<label class="d-flex align-items-center fs-6 fw-bold mb-2">
								<span class="required">Title</span>
								<i class="fas fa-exclamation-circle ms-2 fs-7" data-bs-toggle="tooltip" title="Enter title of the Area"></i>
							</label>
							<div class="validation-div" id="val-title"></div>
							<input type="text" id="title" class="form-control form-control-solid" placeholder="Enter Area Title" />
						</div>
						<div class="row g-9 mb-8">
							<div class="col-md-6 fv-row">
								<label class="required fs-6 fw-bold mb-2">Select City</label>
								<div class="validation-div" id="val-city"></div>
								<select class="form-select form-select-solid" id="city" data-control="select2" data-hide-search="true" data-placeholder="Select City">
						...
						<div class="row g-9 mb-8">
							<div class="col-md-4 fv-row">
								<label class="required fs-6 fw-bold mb-2">Postal/Zip Code</label>
								<div class="validation-div" id="val-postal_code"></div>
								<div class="position-relative d-flex align-items-center">
									<input class="form-control form-control-solid" id="postal_code" placeholder="Ex. 380015" />
								</div>
							</div>
							<div class="col-md-4 fv-row">
								...
								<div class="position-relative d-flex align-items-center">
									<input class="form-control form-control-solid" id="latitude" placeholder="Ex. 12.23456" />
								</div>
							</div>
							<div class="col-md-4 fv-row">
								...
								<div class="position-relative d-flex align-items-center">
									<input class="form-control form-control-solid" id="longitude" placeholder="Ex. 22.54321" />
								</div>
							</div>
						</div>
					...
					<table class="table table-row-dashed table-row-gray-300 align-middle gs-0 gy-4 data-tbl">
						<thead>
							<tr class="fw-bolder text-muted">
								...
								<th class="min-w-30px">Postal/Zip Code</th>
...
<script type="text/javascript">
	...
	// STORE
  	function saveData(){
		...
		data.append('type', 'area');
		data.append('id', $('#formData #id').val());
		data.append('title', $('#formData #title').val());
		data.append('city', $('#formData #city').val());
		data.append('postal_code', $('#formData #postal_code').val());
		data.append('latitude', $('#formData #latitude').val());
		data.append('longitude', $('#formData #longitude').val());
	...
	// EDIT
  	function editThis(id = ''){
		...
		if(response.status == '200' && response.success == '1'){
			...
			$('#formData #postal_code').val(response.data.postal_code);
			$('#formData #latitude').val(response.data.latitude);
			$('#formData #longitude').val(response.data.longitude);
			$('#formData #city').val([response.data.city_id]).trigger('change');
