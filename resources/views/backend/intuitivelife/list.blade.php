	...
	<div class="toolbar" id="kt_toolbar">
		<div id="kt_toolbar_container" class="container-fluid d-flex flex-stack">
			...
			<div class="d-flex align-items-center gap-2 gap-lg-3">
				...
				<a href="{{ route('lifeatinsetive.create') }}" class="btn btn-sm btn-primary">Create</a>
			</div>
		</div>
	</div>
@endsection
...
<script>
	...
	$(function () {
		...
		var table = $('.data-tbl').DataTable({
			...
			serverSide: true,
			ajax: {
				url: "{{route('ajax.company.lifeatinsetive')}}",
				method: "POST",
				"data" : {
					"_token": "{{ csrf_token() }}",
				}
	...
	function deleteThis(item_id = ''){
		if(confirm("{{trans('common.delete_confirm')}}")){
			...
			var response = adminAjax('{{route("ajax.delete.pressrelease")}}', data);
