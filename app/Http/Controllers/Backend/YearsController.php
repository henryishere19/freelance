...
class YearsController extends CommonController
{   
	use CommonHelper;
	...
	*/
	...
	public function index(){
		...
		return view('backend.years.list', compact('page', 'page_title'));
	}
	...
	public function create(){
		...
		$data = Years::create(['title'=> '', 'content' => '']);
		if($data){
			return redirect()->route('years.edit',$data->id);
		}
	...
	public function edit($id = null){
		...
		$data = Years::find($id);
		if($data){
			return view('backend.years.edit',compact('page','page_title', 'data'));
		}
		return redirect()->route('years.index');
	}
				...
				return Datatables::of($data)
					...
					->addColumn('action', function($row){
						   return '<div class="d-flex justify-content-end flex-shrink-0">
										<a href="'. route("years.edit",$row->id) .'" class="btn btn-icon btn-bg-light btn-active-color-primary btn-sm me-1"><i class="fa fa-edit"></i></a>
	...
	public function store(Request $request){
		...
		try{
			...
			if($request->type == "content")
			{
				...
				$data['content']	= $request->content_description;
			...
			if($request->item_id){
				// UPDATE
				$product = Years::find($request->item_id);
	...
	*/
	public function change_status(Request $request){
		DB::beginTransaction();
		try {
			$query = Years::where('id',$request->id)->update(['status'=>$request->status]);
			if($query){
			  DB::commit();
	...
	*/
	public function destroy(Request $request){
		...
		try{
			// DELETE
			$query = Years::where(['id'=>$request->item_id])->delete();
			if($query){
