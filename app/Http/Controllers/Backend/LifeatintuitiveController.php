...
class LifeatintuitiveController extends CommonController
{   
	use CommonHelper;
	...
	*/
	...
	public function index(){
		return view('backend.intuitivelife.list');
	}
	...
	public function create(){
		...
		$data = Lifeatinstetive::create(['title'=> '', 'description' => '']);
		if($data){
			return redirect()->route('lifeatinsetive.edit',$data->id);
		}
		return redirect()->route('lifeatinsetive.index');
	}
	...
	public function edit($id = null){
		...
		$data = Lifeatinstetive::find($id);
		if($data){
			$gallery	= Media::where('reation_id', $data->id)->get();
			return view('backend.intuitivelife.edit',compact('page','page_title', 'data','gallery'));
		}
		return redirect()->route('lifeatinsetive.index');
	}
				...
				return DataTables::of($data)
						...
						->addColumn('action', function($row){
							   return '<div class="d-flex justify-content-end flex-shrink-0">
												<a href="'. route("lifeatinsetive.edit",$row->id) .'" class="btn btn-icon btn-bg-light btn-active-color-primary btn-sm me-1"><i class="fa fa-eye"></i></a>
	...
	*/
	public function change_status(Request $request){
		DB::beginTransaction();
		try {
			$query = Lifeatinstetive::where('id',$request->id)->update(['status'=>$request->status]);
			if($query){
			  DB::commit();
	...
	public function store(Request $request){
		...
		try{
			...
			if($request->item_id){
				// UPDATE
				$product = Lifeatinstetive::find($request->item_id);
				...
				// CREATE
				$return = Lifeatinstetive::create($data);
				if($return){
					DB::commit();
	...
	public function destroy(Request $request){
		...
		try{
			// DELETE
			$query = Lifeatinstetive::where(['id'=>$request->item_id])->delete();
			if($query){
				DB::commit();
