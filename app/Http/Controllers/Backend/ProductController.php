...
{   
	...
	*/
	...
	public function index(){
		...
		return view('backend.product.list', compact('page', 'page_title'));
	}
	...
	public function create(){
		...
		$data = Product::create(['title:en'=> '', 'description:en' => '', 'price' =>'0', 'owner_id' =>$user->id]);
		if($data){
	...
	public function edit($id = null){
		...
		if($data){
			...
			$attributes = VariationGroup::where('status','active')->get();
			$gallery	= Media::where('reation_id', $data->id)->get();
			if($attributes){
				foreach($attributes as $key=> $row){
					$attributes[$key]->is_selected = 0;
					$is_selected = ProductAttribute::where(['attribute_id'=>$row->id, 'product_id'=>$data->id])->first();
					if($is_selected){ $attributes[$key]->is_selected = 1; }
					$variations = Variation::where(['group_id'=>$row->id, 'status'=>'active'])->get();
					if($variations){
						foreach($variations as $vkey=> $variation){
							$productvariation = ProductVariation::where(['variation_id'=>$variation->id, 'product_id'=>$data->id])->first();
							if($productvariation){
								$variations[$vkey]->price 		= $productvariation->price;
								$variations[$vkey]->sale_price  = $productvariation->sale_price;
							}
						}
						$attributes[$key]->variations = $variations;
					}
				}
			}
			return view('backend.product.edit',compact('page','page_title', 'data','categories', 'brands', 'attributes','gallery'));
		}
		return redirect()->route('products.index');
	}
				...
				return Datatables::of($data)
					...
					->addColumn('action', function($row){
						   return '<div class="d-flex justify-content-end flex-shrink-0">
										<a href="'. route("products.edit",$row->id) .'" class="btn btn-icon btn-bg-light btn-active-color-primary btn-sm me-1"><i class="fa fa-edit"></i></a>
	...
	public function store(Request $request){
		...
		if(!empty($request->offer_price)){
			if($request->offer_price >= $request->price){
				$this->ajaxError([], trans('product.offer_price_can_not_greater_than_price'));
			}
		}
	...
	*/
	public function change_status(Request $request){
		DB::beginTransaction();
		try {
			$query = Product::where('id',$request->id)->update(['status'=>$request->status]);
			if($query){
			  DB::commit();
